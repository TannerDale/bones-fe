class DogWithPlayDatesPoro
  ...
  def initialize(data)
    ...
    @accepted_play_dates = play_data_poros(data[:attributes][:accepted_play_dates])
    @pending_play_dates = play_data_poros(data[:attributes][:pending_play_dates])
  end

  private

  def play_data_poros(data)
    data.map do |pd|
      PlayDatePoro.new(pd)
    end
  end
